...
function GetDate03(jsonDate) {
    strDate = moment(jsonDate).format("MM.DD HH:mm:ss");
    return strDate;
}
...
function CardInfoColor(str) {
    var strArry = str.split(',');
    var returnStr = "";
    for (var i = 0; i < (strArry.length - 1); i++) {
        if (strArry[i].match("◆") || strArry[i].match("♥")) {
            returnStr += "<span style='color:red;'>" + strArry[i] + ",</span>";
        } else {
            returnStr += "<span>" + strArry[i] + ",</span>";
        }
    }
    return returnStr;
}